...
try {
    ...
    $location = isset($_SESSION['request_event_data']['location']) ? $_SESSION['request_event_data']['location'] : '';
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Additional validation
        if (!$title || !$description || !$location || !$eventStart || !$eventEnd) {
        ...
        $stmt = $pdo->prepare("INSERT INTO events (user_id, title, description, location, duration, status, event_start, event_end) VALUES (?, ?, ?, ?, ?, 'pending', ?, ?)");
        $stmt->execute([$_SESSION['user_id'], $title, $description, $location, $duration, $eventStart, $eventEnd]);
